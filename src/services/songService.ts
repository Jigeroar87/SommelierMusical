...
export const songService = {
  async getSongs() {
    ...
    console.log('[songService] Supabase configured:', !!import.meta.env.VITE_SUPABASE_URL, !!import.meta.env.VITE_SUPABASE_ANON_KEY);
    
    const { data, error } = await supabase
      .from('songs')
      .select('*')
      .eq('status', 'official_sommelier')
  ...
  async requestSong(request: {
    ...
    // Explicitly handle user_id as null if not provided for anonymous requests
    const insertData = {
    ...
    // Blind insert (no .select()) to avoid RLS issues where write is allowed but read is not
