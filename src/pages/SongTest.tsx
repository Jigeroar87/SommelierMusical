...
import { ArrowLeft, Loader2, Play, ChevronRight, Sparkles, CheckCircle2, Music } from 'lucide-react';
...
export function SongTest() {
  ...
  const calculateScore = () => {
    ...
    questions.forEach(q => {
      ...
      if (selectedOption && q.weight_map[selectedOption]) {
        totalScore += q.weight_map[selectedOption];
      }
    });
    return totalScore;
  };
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#140F12] text-center p-6">
        ...
        <Button onClick={() => navigate(`/songs/${slug}`)} variant="outline" className="border-[#C8A96B]/20 text-[#C8A96B]">
          Volver al Detalle
        </Button>
      </div>
    );
          ...
          >
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="space-y-2">
                <span className="text-[10px] tracking-[0.5em] uppercase text-[#C8A96B] font-bold">Cata Completada</span>
            ...
            <div className="glass p-12 space-y-10 relative overflow-hidden bg-[#C8A96B]/[0.02]">
              ...
              <div className="space-y-4">
                <span className="text-[11px] tracking-[0.5em] uppercase text-[#F3EBDD]/40 font-bold">Intensidad Emocional Detectada</span>
                <div className="text-7xl font-serif italic text-[#C8A96B]">{totalScore}</div>
                <div className="text-[10px] tracking-[0.2em] uppercase text-[#F3EBDD]/20 font-bold">Sobre un total de {questions.length * 10} unidades</div>
              </div>
              ...
              <div className="space-y-6 text-left">
                <div className="space-y-2">
                  <span className="text-[10px] tracking-[0.3em] uppercase text-[#C8A96B] font-bold">Lectura Sommelier</span>
                  <p className="text-xl font-serif italic text-[#F3EBDD]/80 leading-relaxed">
                    "{song.sommelier_phrase}"
                  </p>
                </div>
                <div className="space-y-2">
                  <span className="text-[10px] tracking-[0.3em] uppercase text-[#F3EBDD]/40 font-bold">Herida Primordial</span>
    ...
    <div className="min-h-screen flex flex-col bg-[#140F12] relative overflow-hidden text-[#F3EBDD]">
      ...
      <main className="flex-grow w-full max-w-4xl mx-auto px-6 py-20 relative z-10 flex flex-col">
        <header className="flex justify-between items-center mb-20">
          ...
          >
            <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Abandonar
          </button>
        ...
        <div className="flex-grow flex flex-col justify-center gap-16">
            ...
            >
              ...
              <div className="grid gap-4">
                {currentQuestion.options.map((option) => (
                  ...
                  >
                    {!answers[currentQuestion.id] && <div className="glossy-overlay opacity-30 group-hover:opacity-100 transition-opacity" />}
                    <div className="flex items-center justify-between relative z-10">
                      <span className="text-xl font-serif italic">{option}</span>
                      {answers[currentQuestion.id] === option && <CheckCircle2 size={24} />}
                    </div>
                  </button>
                ))}
              </div>
          ...
          <footer className="flex justify-end pt-8">
            <Button
              ...
              className={`min-w-[200px] border-[#C8A96B] ${!answers[currentQuestion.id] ? 'opacity-30' : 'bg-[#C8A96B] text-[#140F12] hover:bg-[#F3EBDD]'}`}
            >
